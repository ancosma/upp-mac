...
Size BarPane::Repos(bool _horz, int maxsize)
{
	...
	LLOG("BarPane::Repos start " << Name() << " " << asz);
	while(q < item.End()) {
		Size psz(0, 0);
	...
	LLOG("BarPane::Repos end " << Name() << " " << asz);
	return asz;
}
...
{
		LLOG("BarPane::Add ");		//aris002
	Item& m = item.Add();
	m.ctrl = ctrl;
	m.gapsize = gapsize;
	if(ctrl)
	{
		LLOG("BarPane::Add if(ctrl) AddChild");		//aris002		
		AddChild(ctrl);
	}
}
...
void BarCtrl::Layout()
{
	LLOG("BarCtrl::Layout");
	if(IsChild()) {
		LLOG("    BarCtrl::Layout if(IsChild())");		
