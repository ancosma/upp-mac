...
Bar::Item& MenuBar::AddItem(Callback cb)
{
	...
	{
		q = new TopMenuItem;
	LLOG("MenuBar::AddItem TopMenuItem " << Name());		
	}
	else
	{
		q = new MenuItem;
	LLOG("MenuBar::AddItem MenuItem " << Name());		
	}
	item.Add(q);
	LLOG("MenuBar::AddItem item.Add(q); ");		
	
	pane.Add(q, Null);
	LLOG("MenuBar::AddItem pane.Add(q); ");		
...
void MenuBar::PopUp(Ctrl *owner, Point p, Size rsz)
{
	...
	restorefocus = GetFocusCtrl();
	LLOG("+++++++++++++++MenuBar::PopUp " << UPP::Name(this) << " set restorefocus:" << UPP::Name(restorefocus));
	DistributeAccessKeys();
	...
	LLOG("+++++++++++++++MenuBar::PopUp after pane.SubMenu(); sz=" <<sz);
...
//		SetRect(szx ? p.x + sz.cx : p.x, szy ? p.y + sz.cy : p.y, parentmenu ? sz.cx : 1, 1);
	LLOG("+++++++++++++++MenuBar::PopUp after GUIEFFECT_SLIDE=" <<GetRect()<<" sz="<<GetSize() );
		
	}
	else
	{
		SetRect(p.x, p.y, sz.cx, sz.cy);
//		SetRect(p.x, p.y, 50, 50);
	LLOG("+++++++++++++++MenuBar::PopUp after else GUIEFFECT_SLIDE=" <<GetRect());
