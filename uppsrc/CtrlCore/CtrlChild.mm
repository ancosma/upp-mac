...
void  Ctrl::AddChild(Ctrl *q, Ctrl *p)
{
	GuiLock __;
	ASSERT(q);	
	LLOG("Ctrl::AddChild " << UPP::Name(q) << " to: " << Name());  //aris002
	NSLog(@"Ctrl::AddChild ");
	if(p == q) return;
	...
	if(q->parent) {
		ASSERT(!q->inframe);
		if(q->parent == this) {
		LLOG("!!!! Ctrl::AddChild b4 RemoveChild0 "<< q->GetRect());  //aris002			
			RemoveChild0(q);
			updaterect = false;
		}
		else
	...
	NSLog(@"Ctrl::AddChild 1");
	
//	LLOG("!!!! Ctrl::AddChild b4 q->parent = this; "<< q->GetRect());  //aris002	
	q->parent = this;
	NSLog(@"Ctrl::AddChild 2");
	
//	LLOG("!!!! Ctrl::AddChild after q->parent = this; "<< q->GetRect()<<" "<<q->parent->GetRect());  //aris002
	...
	if(updaterect)
		q->UpdateRect();
	LLOG("!!!! Ctrl::AddChild b4 ChildAdded "<< q->GetRect()<<" "<<q->parent->GetRect());  //aris002
	ChildAdded(q);
	q->ParentChange();
	if(updaterect && GetTopCtrl()->IsOpen())		
	{
		q->StateH(OPEN);
	LLOG("!!!!! Ctrl::AddChild after  q->StateH(OPEN);"<<q->GetRect());  //aris002

	}
	NSLog(@"Ctrl::AddChild 3");
...
{
	GuiLock __;
	LLOG("*Ctrl::GetMainWindow() Ctrl *q = GetTopCtrl();");
	Ctrl *q = GetTopCtrl();
	for(;;) {
		Ctrl *w = q->GetOwner();
		if(!w)
...
{
	LLOG("*Ctrl::GetMainWindow() return const_cast");
