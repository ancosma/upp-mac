...
bool SystemDraw::ExcludeClipOp(const Rect& r)
{
	...
	//	::DeleteObject(hrgn);
	//#endif
	return false;//q == SIMPLEREGION || q == COMPLEXREGION;
}
...
bool SystemDraw::IsPaintingOp(const Rect& r) const
{
	...
	return true;  //::RectVisible(handle, r);
}
...
void SystemDraw::DrawRectOp(int x, int y, int cx, int cy, Color color)
{
	...
	CGContextSetRGBFillColor(handle, color.GetR(),color.GetG(),color.GetB(), 1);  //1 is opaque
	CGContextFillRect(handle, CGRectMake(x, y, cx, cy));
...
void SystemDraw::DrawLineOp(int x1, int y1, int x2, int y2, int width, Color color)
{
    ...
    CGContextSetRGBFillColor(handle, color.GetR(),color.GetG(),color.GetB(), 1);  //1 is opaque
    CGContextSetRGBStrokeColor(handle, color.GetR(),color.GetG(),color.GetB(), 1);
	
    assert(CGContextIsPathEmpty(handle));
	
	
	//    CGContextSaveGState(context);
    CGContextSetLineWidth(handle, width);
    CGContextSetShouldAntialias(handle, 0);
	
	//    CGContextSaveGState(context);
	
	//    CGContextTranslateCTM(context, 100, 100);
	
    CGContextMoveToPoint(handle, x1, y1);
	CGContextAddLineToPoint(handle, x2, y2);
